<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">My Records</h2>
            <p class="text-sm text-slate-500 mt-1">Manage and track your submitted sales and receipts.</p>
        </div>
        <a href="<?= url('/sales/create') ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-sm font-semibold transition-all duration-200 shadow-sm">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div class="bg-amber-50 p-4 rounded-2xl border border-amber-100 shadow-sm">
            <p class="text-xs font-medium text-amber-600 uppercase tracking-wider mb-1">Pending</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="relative max-w-sm w-full">
                ...
                <input type="text" id="searchInput" placeholder="Search records..." class="block w-full pl-10 pr-3 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 bg-white">
            </div>

            <form method="GET" action="<?= url('/records') ?>" class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        ...
                        <td colspan="6" class="px-6 py-10 text-center text-slate-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <?php if ($filters['status'] !== 'all' || $filters['month'] !== 'all'): ?>
                                    ...
                                    <a href="<?= url('/records') ?>" class="text-brand-600 hover:text-brand-700 text-xs font-medium mt-2">Clear Filters</a>
                                <?php endif; ?>
                            </div>
                        </td>
                    </tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex items-center gap-1 text-slate-400">
                                        ...
                                        <span class="text-[10px] font-bold"><?= $record['comment_count'] ?></span>
                                    </div>
                                    <?php endif; ?>
                                </div>
                            </td>
